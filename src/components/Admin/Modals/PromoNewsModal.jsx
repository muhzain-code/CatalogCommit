...
const PromoNewsModal = ({ isOpen, onClose, onSave, news }) => {
    ...
    useEffect(() => {
        if (news) {
            setFormData({
                ...
                promoProducts: news.promoProducts || [],
            })
        } else {
            setFormData({
                name: "",
    ...
    const handleProductChange = (product, checked) => {
        if (checked) {
            setFormData({
                ...formData,
                promoProducts: [...formData.promoProducts, product],
            })
        } else {
            setFormData({
                ...formData,
                promoProducts: formData.promoProducts.filter((p) => p !== product),
            })
        }
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold">{news ? "Edit Berita Promo" : "Tambah Berita Promo"}</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Produk Promo</label>
                            ...
                            {availableProducts.map((product) => (
                                <label key={product} className="flex items-center">
                                    <input
                                        ...
                                        checked={formData.promoProducts.includes(product)}
